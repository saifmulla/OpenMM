...
KernelImpl* OpenCLKernelFactory::createKernelImpl(std::string name, const Platform& platform, ContextImpl& context) const {
    ...
    if (name == ControlBinForcesKernel::Name())
        return new OpenCLControlBinForcesKernel(name,platform,cl);
    if (name == MeasureBinVirialKernel::Name())
	return new OpenCLMeasureBinVirialKernel(name,platform,cl);
